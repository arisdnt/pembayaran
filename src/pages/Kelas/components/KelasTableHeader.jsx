...
export function KelasTableHeader({
  searchQuery,
  setSearchQuery,
  filterTingkat,
  setFilterTingkat,
    ...
    <div className="border-b border-slate-200/80 bg-gradient-to-r from-slate-50 to-white">
      <div className="flex flex-wrap items-center gap-4 px-6 py-4">
        <div className="flex-1 min-w-[240px] max-w-xs">
          <TextField.Root
            placeholder="Cari tingkat atau kelas..."
            value={searchQuery}
        ...
        <div className="flex items-center gap-2">
          <Select.Root value={filterTingkat} onValueChange={setFilterTingkat}>
            ...
            <Select.Content style={{ borderRadius: 0 }}>
              ...
              {stats.tingkatList.map(tingkat => (
                <Select.Item key={tingkat} value={tingkat}>Tingkat {tingkat}</Select.Item>
              ))}
            </Select.Content>
          </Select.Root>
        </div>
        ...
        <div className="ml-auto flex items-center gap-2 text-xs">
          ...
          <div className="flex items-center gap-1.5 px-2.5 py-1.5 bg-blue-50 border border-blue-200">
            <span className="text-blue-700">Tingkat:</span>
            <span className="font-semibold text-blue-900">{stats.tingkatList.length}</span>
          </div>
          {hasActiveFilters && (
            <div className="flex items-center gap-1.5 px-2.5 py-1.5 bg-emerald-50 border border-emerald-200">
              <span className="text-emerald-700">Ditampilkan:</span>
              <span className="font-semibold text-emerald-900">{stats.filtered}</span>
            </div>
          )}
        </div>
