import { TagihanDetailSkeleton } from './TagihanDetailSkeleton'
import { TagihanDetailEmpty } from './TagihanDetailEmpty'
...
export function DetailPanel({ selectedItem, isLoading = false, isRefreshing = false }) {
  if (isLoading) {
    return <TagihanDetailSkeleton />
  }

  if (!selectedItem) {
    return <TagihanDetailEmpty />
  }
  ...
  return (
    <div className="relative flex h-full flex-col border border-slate-300 bg-white shadow-lg">
      ...
      <div className="flex-1 overflow-auto p-4">
        <TagihanDetailInfo tagihan={selectedItem} />
      </div>
    </div>
  )
}
